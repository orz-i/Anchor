...
async fn handle_tools_call(
    ...
    let legacy_facade_leaf = crate::tools::registry::is_legacy_facade_tool(name);
    if !crate::skills::is_skill_tool(name)
        && !legacy_facade_leaf
        && state.is_published_tool(name) == Some(false)
    {
...
mod tests {
    ...
    fn tools_list_returns_a_budget_compliant_catalog_in_one_response() {
        ...
        assert_eq!(first["_meta"]["anchor/catalog"]["local_tool_count"], 27);
    ...
    fn advanced_tools_list_keeps_browser_recovery_tools_on_the_first_page() {
        ...
        for legacy in [
            ...
            assert!(
                !first_names.contains(legacy),
                "legacy leaf {legacy} leaked into advanced tools/list"
            );
        }
        assert_eq!(
    ...
    fn initialize_instructions_define_the_history_persistence_workflow() {
        ...
        assert!(instructions.contains("legacy Skill helper tools are intentionally not published"));
    ...
    #[tokio::test]
    async fn domain_facades_are_published_while_legacy_leaf_routes_remain_callable() {
        let workspace = tempfile::tempdir().expect("workspace tempdir");
        let harness = tempfile::tempdir().expect("harness tempdir");
        let mut context =
            ToolContext::for_test(workspace.path().to_path_buf(), harness.path().to_path_buf())
        ...
        {
            assert!(
                !tool_names.contains(legacy),
                "legacy leaf {legacy} leaked into tools/list"
            );
        }

        let legacy_git = handle_request(
            &state,
            &json!({
                "jsonrpc":"2.0",
                "id":2,
        ...
        .await;
        assert_eq!(legacy_git["result"]["structuredContent"]["ok"], true);

        let legacy_task = handle_request(
            &state,
            &json!({
                "jsonrpc":"2.0",
                "id":3,
        ...
        .await;
        assert_eq!(legacy_task["result"]["structuredContent"]["ok"], true);
    ...
    async fn skills_are_discovered_and_loaded_through_mcp() {
        ...
        let tool_names = tools["result"]["tools"]
            ...
            .filter_map(|tool| tool["name"].as_str())
            .collect::<Vec<_>>();
        for legacy in crate::skills::TOOL_NAMES {
        ...
        // Legacy helper handlers remain callable for clients that cached their
        // schemas before native Skill import was enabled, despite being hidden
        // from tools/list.
