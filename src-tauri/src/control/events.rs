...
mod tests {
    ...
    async fn event_batches_bound_hostile_content_and_frame_growth() {
        ...
        let response = super::super::protocol::ControlResponse::success(
            "hostile-events".into(),
            super::super::protocol::ControlResult::Events { batch },
        );
        let encoded = serde_json::to_vec(&response).expect("serialize event response");
