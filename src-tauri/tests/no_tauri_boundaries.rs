...
#[test]
fn package_and_lockfiles_have_no_tauri_dependencies_or_scripts() {
    ...
    for path in ["pnpm-lock.yaml", "package-lock.json"] {
        let lock = read(path);
        assert!(
            !lock.contains("@tauri-apps"),
            "Tauri package remains in {path}"
        );
    }
}
...
fn active_frontend_and_rust_sources_have_no_tauri_runtime_references() {
    ...
    collect_source_files(&root.join("src-tauri/src"), &mut files);
...
fn compatibility_data_path_is_not_rewritten_by_tauri_removal() {
    let brand = read("src-tauri/src/brand.rs");
