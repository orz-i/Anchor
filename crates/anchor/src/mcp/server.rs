...
fn attach_proxy_auto_checkpoint(
    ...
    match crate::tools::session::auto_checkpoint_after_tool(
        ctx,
        tool_name,
        arguments,
        result,
        ...
        Ok(Some(checkpoint)) => {
                ...
                .and_then(Value::as_object_mut)
            {
                structured.insert(
                    "checkpoint".into(),
                    crate::tools::session::checkpoint_reference(&checkpoint),
                );
            }
        }
        Ok(None) => {}
...
mod tests {
    ...
    fn initialize_instructions_define_the_history_persistence_workflow() {
        ...
        assert!(instructions.contains("`session` facade with operation=open exactly once"));
        assert!(instructions.contains("isolated development Session"));
        assert!(instructions.contains("must not read, summarize, inject, pause"));
        assert!(instructions.contains("session operation=list"));
        assert!(instructions.contains("operation=get"));
        assert!(instructions.contains("session_id and session_path"));
        ...
        assert!(instructions.contains("explicit Session checkpoints reject"));
        assert!(instructions.contains("After completing each user-requested task"));
        assert!(instructions.contains("session operation=checkpoint"));
    ...
    #[test]
    fn workspace_prompt_initializes_an_isolated_chatgpt_session() {
        let component = include_str!("../../../../src/components/admin/ChatGptSessionPrompt.tsx");

        assert!(component.contains("ChatGPT 新会话启动提示词"));
        assert!(component.contains("初始化本 ChatGPT 会话对应的独立开发 Session"));
        assert!(component.contains("session，operation=open"));
        assert!(component.contains("Session 默认自然隔离"));
        assert!(component.contains("operation=list"));
        assert!(component.contains("operation=get"));
        assert!(component.contains("session_id"));
        assert!(component.contains("session_path"));
        assert!(component.contains("expected_path"));
        assert!(component.contains("operation=checkpoint"));
    ...
    #[test]
    fn chatgpt_session_metadata_is_injected_only_for_session_initialization_and_work_session() {
        let params = json!({
            "arguments": {"operation": "open"},
            "_meta": {"openai/session": "chatgpt-conversation"}
        });
        let session = tool_arguments("session", &params);
        assert_eq!(session["operation"], "open");
        assert_eq!(session["_host_session_key"], "chatgpt-conversation");
    ...
    #[tokio::test]
    async fn chatgpt_host_session_maps_to_one_opaque_isolated_session() {
        let workspace = tempfile::tempdir().expect("workspace tempdir");
        let harness = tempfile::tempdir().expect("harness tempdir");
        let state = Arc::new(
            ToolContext::for_test(workspace.path().to_path_buf(), harness.path().to_path_buf())
