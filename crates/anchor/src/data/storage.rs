...
mod tests {
    ...
    #[test]
    fn profiles_v1_are_hard_rejected() {
        let temp = tempfile::tempdir().expect("tempdir");
        let path = temp.path().join("profiles.json");
        let data = AppData::default();
        let mut value =
        ...
        let error = load_profiles_with_backup(&path).expect_err("v1 profiles must fail");
        assert!(error
            .to_string()
            .contains("不支持的配置 schema_version：1；当前仅支持 2"));
    }

    #[test]
    fn secrets_v1_are_hard_rejected() {
        let mut value = serde_json::to_value(SecretsData::default()).expect("secrets json");
        value["schema_version"] = serde_json::Value::from(1);
        let plaintext = serde_json::to_vec(&value).expect("v1 secrets");

        let error = parse_secrets_payload(&plaintext).expect_err("v1 secrets must fail");
        assert!(error
            .to_string()
            .contains("不支持的凭据内容 schema_version：1；当前仅支持 2"));
    }
...
fn load_profiles_with_backup(path: &Path) -> AppResult<ProfilesData> {
    match read_profiles_json_versioned(path) {
        ...
        Err(primary_error) => {
            ...
            let recovered = read_profiles_json_versioned(&backup).map_err(|backup_error| {
                crate::error::AppError::Message(format!(
                    "配置文件损坏且备份无法读取：主文件错误：{primary_error}；备份错误：{backup_error}"
                ))
            })?;
...
fn read_profiles_json(path: &Path) -> AppResult<ProfilesData> {
    read_profiles_json_versioned(path)
}

fn read_profiles_json_versioned(path: &Path) -> AppResult<ProfilesData> {
    let raw = fs::read_to_string(path)?;
