...
#[test]
fn frontend_has_no_retired_framework_dependencies() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(
        violations.is_empty(),
        "retired frontend/runtime imports remain in the Web Admin: {violations:?}"
    );
}

#[test]
fn removed_desktop_shell_cannot_own_runtime_resources() {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    assert!(!manifest.join("src/app_state.rs").exists());
    assert!(!manifest.join("src/legacy_desktop.rs").exists());
    assert!(!manifest.join("src/commands").exists());
    let lib = fs::read_to_string(manifest.join("src/lib.rs")).expect("core lib");
    assert!(!lib.contains("tauri::"));
    assert!(!lib.contains("legacy_desktop"));
}
