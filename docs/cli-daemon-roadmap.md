...
- daemon state 已 hard-cut 到 schema v3：运行态只保留 `tunnelServices=mcp|all|null`，不再保留或解释旧 `tunnel: bool`；内部 daemon 引导也只使用可选 `--tunnel-service`，旧 `--tunnel` / `--no-tunnel` 不再接受。当前公开 CLI 已将 Tunnel 提升为顶级资源，Workspace 启动参数不再承担 Tunnel ownership 配置。
...
- 普通管理能力已继续补齐：Workspace 创建/删除、目录打开、Skill inspection、Health checks、顶级 Tasks 聚合/Task snapshot、FRP 非敏感 metadata 保存与 profile 删除均进入共享 `management.rs` 并由 Web dispatcher 暴露；Tasks 统一跨 Workspace 展示并支持 Workspace 筛选，旧 Workspace `/canvs` 页面与 Workspace Detail Canvs tab 已硬切移除；FRP metadata 与 Token 写入已经拆成两个独立权限域，浏览器先持久化非敏感 metadata，再对稳定 profile ID 单独执行高权限 Token 确认；
