...
- `service-run` 是 OS service manager 内部入口，不应直接调用；Windows SCM 形态为 `config-dir + owner SID + username`，Linux systemd 形态仅为 `config-dir`；
