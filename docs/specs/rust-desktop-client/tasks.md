...
- [x] 1.1 初始化 Tauri 2 + Svelte 工程，验证 `cargo tauri dev` 可启动空白窗口
  - **证据块**: `docs/project-context/architecture.md` 目标目录结构与当前 Tauri/Svelte 工程
...
- [x] 1.2 实现 WorkspaceProfile 数据模型与 JSON 持久化
  - **证据块**: `src-tauri/src/workspace/model.rs` 与 `src-tauri/src/data/` 的当前 WorkspaceProfile 和持久化契约
...
- [x] 1.3 实现系统钥匙串密钥存储，替代旧版 secrets.json
  - **证据块**: `src-tauri/src/data/secret_protection.rs` 与 `src-tauri/src/data/storage.rs` 的受保护凭据实现
...
- [x] 2.2 实现内嵌 MCP HTTP server（axum），支持 initialize / tools/list / tools/call
  - **证据块**: `src-tauri/src/mcp/protocol.rs` 与 `src-tauri/src/mcp/listener.rs` 的协议和 Streamable HTTP 契约
...
- [x] 3.1 实现 FRP 配置生成和 Cloudflare 隧道进程监督
  - **证据块**: `src-tauri/src/tunnel/` 与 `src-tauri/src/workspace/model.rs`
...
- [x] 3.2 实现 OAuth / Bearer / NoAuth 认证配置
  - **证据块**: `src-tauri/src/auth/` 与 `src-tauri/src/workspace/model.rs` 的 OAuth/AuthConfig 实现
...
- [x] 3.3 实现健康检查（本地 /mcp、公网 endpoint、OAuth 元数据）
  - **证据块**: `src-tauri/src/health/checker.rs` 与健康检查合约测试
...
- [x] 4.1 实现 Workspace 卡片首页和详情页布局
  - **证据块**: `src/routes/`、`src/lib/components/` 与 `docs/design-system.md`
...
- [x] 4.2 实现配置表单、健康面板、日志查看器和一键复制
  - **证据块**: 当前 Workspace 页面、配置表单、健康面板与日志组件
...
- [x] 5.1 移植 P0 工具合规测试（mcp_contract + tool_golden + security 路径穿越）
  ...
  - **证据块**: `src-tauri/tests/call_tool_contract.rs`、`call_tool_security.rs` 与 `tool_output_schema_contract.rs`
