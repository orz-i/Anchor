...
Anchor 是 **Rust CLI/daemon + Vite/React Web Admin** 的本地 Workspace/MCP 控制平面。`anchor` CLI 是唯一 Rust 产品可执行目标；Web Admin 由 `anchor admin` 在 loopback 托管并嵌入生产静态资源。Tauri desktop、IPC adapter、desktop bin、bundle 配置和安装包构建链已经物理删除。

## 主执行链路

```text
Browser React Web Admin
...
- `src/`：Vite + React Router + shadcn/ui Web Admin，零 `@tauri-apps` 依赖。
- `crates/anchor/src/bin/anchor.rs`：唯一产品 CLI 入口。
- `crates/anchor/src/admin.rs` / `admin_daemon.rs` / `admin_service.rs`：本机 Web 管理 API 与持久托管。
- `crates/anchor/src/management.rs`：CLI/Web Admin 共用管理语义。
- `crates/anchor/src/daemon.rs` / `control/`：Workspace runtime 权威。
- `crates/anchor/src/gateway_daemon.rs` / `gateway_control/`：Gateway runtime 权威。
- `crates/anchor/src/windows_service.rs`：Windows SCM supervisor 与 owner-token 启动边界。
- `crates/anchor/src/tools/` / `harness/`：开发工具、安全策略、Task/verification/journal。
