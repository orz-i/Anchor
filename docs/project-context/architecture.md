...
`management.rs` 是 CLI/Web Admin 共用的管理语义边界。除读取能力外，Workspace 创建/删除、目录打开、Skill inspection、Health、顶级 Tasks 聚合、FRP 非敏感 metadata CRUD 均由共享层提供；`tasks.rs` 只从既有 `TaskHarness` / `CodingHarness` 投影 Task 列表、事件、操作、ChangeSet 与 Verification，并以 Workspace 作为筛选维度，不建立第二套任务存储或生命周期状态。Task detail 的最近事件/操作使用 Harness newest-first bounded read；不会为了展示固定数量的最近记录先把 retained journal 以 `usize::MAX` 全量物化到内存，其中 operation 在存储读取阶段就按 Task 过滤。Workspace 配置通过 CLI 共用的 `preview/stage/apply` 事务写入，浏览器使用 `baseProfile` 做 optimistic concurrency guard；MCP runtime 与 Tunnel start/restart/stop/test 都只通过 Workspace daemon 控制域执行。Gateway 配置热应用、关闭和 reload 同样进入共享层；per-workspace route selector 使用 Gateway control protocol 的 additive `set_routes`，运行中的 Gateway 在同一 daemon PID 内切换 route 集合并在失败时恢复旧运行态，不允许 HTTP 直接写 Gateway runtime。HTTP handler 只负责安全校验、参数反序列化和调用共享层，不持有 daemon/Tunnel/Gateway 运行权威。

Web Admin 的 Harness 任务视图是顶级 `/tasks` 页面：统一列出所有已注册 Workspace 的当前与历史 Task，可按 Workspace、状态和关键词筛选，并按选中 Task 读取操作、验证、事件和 ChangeSet。Workspace Detail 不再包含 Canvs/Task tab；Workspace MCP listener 也不再发布 `/canvs`、`/canvas` 或对应 JSON/HTML 页面，任务观察只经本机 Admin 管理面进入。
...
- **Tunnel authority**: `WorkspaceProfile` 只保存 Workspace 自身配置，不再携带 `tunnel` / `tunnelId` / enabled / revision 等运行时投影。需要同时访问 Workspace 与其可选 MCP Tunnel 的运行路径只临时构造非持久化 `WorkspaceRuntimeContext { workspace, tunnel }`；Tunnel 配置、revision、启用状态和 Secret 的唯一 authority 始终是顶级 `TunnelProfile`。
...
- **Worktree Task 启动**: Task ID 的分配和 Task 创建只经过 `TaskHarness`；创建成功后才由 `CodingHarness::attach_git_worktree` 绑定 managed worktree 并切换到该 execution scope 的 baseline。`CodingHarness` 不再暴露 `start_task_in_git_worktree` 之类的 Task 创建入口。attach 失败时启动流程必须终止刚创建的 Task，并清理由本次调用创建的 managed worktree；任一 rollback 失败都显式报错，不静默留下第二个可写权威。
