...
test("Tunnel 表单仅在持久化配置或工作区上下文真实变化时重置 draft", async () => {
  ...
  const dependencyLine = source
    .split("\n")
    .find((line) => line.includes("}), [workspaceId, config.type"));
  ...
  assert.match(source, /scope: `\$\{workspaceId\}:mcp`/, "Tunnel 表单应固定到现役 MCP 服务域");
});
...
test("普通 Tunnel 保存只持久化配置，不显式 start/restart/stop tunnel", async () => {
  ...
  assert.doesNotMatch(source, /saveActionsTunnel|Actions Tunnel/, "已移除的 Actions Tunnel 保存路径不得回流");
...
test("显式启动和测试仍会先保存最新 Tunnel draft", async () => {
  ...
  assert.match(source, /if \(dirty\) await saveDraft\(\); const result = await startTunnel\(workspaceId, "mcp"\)/, "启动 Tunnel 前应保存最新 draft");
  assert.match(source, /if \(dirty\) await saveDraft\(\); const result = await testTunnel\(workspaceId, "mcp"\)/, "测试 Tunnel 前应保存最新 draft");
});
