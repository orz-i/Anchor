...
export function TunnelConfigForm({ workspaceId, config, onSave }: { workspaceId: string; config: TunnelFormConfig; onSave: (config: TunnelFormConfig) => void | Promise<void> }) {
  const persisted = useMemo(() => ({
    scope: `${workspaceId}:mcp`,
    config: {
      type: config.type,
  ...
  }), [workspaceId, config.type, config.public_url, config.frp_server, config.frp_subdomain, config.frp_profile_id, config.frp_server_port, config.frp_proxy_type, config.frp_cert_path, config.frp_key_path, config.cloudflare_mode, config.use_proxy]);
  ...
  const secretKey: WorkspaceSecretKey = showFrp ? "frp_token" : "cloudflare_token";
  ...
  return <form className="grid gap-5" onSubmit={(event) => { event.preventDefault(); void save(); }}><FieldGroup>
    ...
    <Field><FieldLabel htmlFor="mcp-public-url">公网 URL</FieldLabel><Input id="mcp-public-url" type="url" className="font-mono" value={draft.public_url} placeholder="https://..." onChange={(event) => update("public_url", event.target.value)} /><FieldDescription>FRP 控制连接地址与实际公网 URL 相互独立。</FieldDescription></Field>
