...
>
  <div class="flex items-start justify-between gap-5 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-4 py-3">
    <div class="min-w-0">
      <p class="text-sm font-medium text-[var(--color-text-primary)]">通过 MCP 提供 Agent Skills</p>
    ...
    <label class="inline-flex shrink-0 items-center gap-2 text-xs text-[var(--color-text-secondary)]">
  ...
  <label class="grid gap-1.5">
    <span class="text-xs text-[var(--color-text-muted)]">Skill 根目录（每行一个）</span>
    <textarea
      class="min-h-32 resize-y rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 font-mono text-xs leading-5 disabled:opacity-50"
  ...
  {#if inspection}
    <div class="grid gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] p-3">
      ...
      {#if inspection.skills.length}
        <div class="grid max-h-64 gap-2 overflow-y-auto pr-1">
          {#each inspection.skills as skill}
            <div class="rounded-md border border-[var(--color-border)] px-3 py-2.5">
              <div class="flex flex-wrap items-baseline justify-between gap-2">
                <code class="font-mono text-xs font-semibold text-[var(--color-text-primary)]">{skill.name}</code>
      ...
      {#if inspection.warnings.length}
        <div class="grid gap-1 border-t border-[var(--color-border)] pt-2">
          {#each inspection.warnings as warning}
            <p class="text-xs leading-5 text-[var(--color-warning)]">{warning}</p>
          {/each}
        </div>
      {/if}
    </div>
